...
cols = ["statuses_count", "followers_count", "followees_count", "favorites_count", "listed_count", "median_int",
        "average_int", "betweenness", "eigenvector", "in_degree", "out_degree"] + \
       ["{0}_glove".format(v) for v in range(384)] + \
       ["sentiment", "subjectivity", "traveling_empath", "fashion_empath", "sadness_empath", "fun_empath",
...
for train_index, test_index in skf.split(X, y):
    ...
    # svc = GradientBoostingClassifier(n_estimators=1000)
    # svc = RandomForestClassifier(n_estimators=1000,  class_weight={0: 1, 1: 10})
    svc = svm.SVC(kernel='rbf', cache_size=7000, class_weight={0: 1, 1: 10})
